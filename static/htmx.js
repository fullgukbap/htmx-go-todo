            ...
            if (Wr === "every") {
              ...
              for (en && (tn.eventFilter = en); Gr.length > 0 && Gr[0] !== ","; ) {
                ...
                if (rn === "changed")
                  ...
                  tn.from = nn;
                } else rn === "target" && Gr[0] === ":" ? (Gr.shift(), tn.target = tt(Gr)) : rn === "throttle" && Gr[0] === ":" ? (Gr.shift(), tn.throttle = d(y(Gr, x))) : rn === "queue" && Gr[0] === ":" ? (Gr.shift(), tn.queue = y(Gr, x)) : rn === "root" && Gr[0] === ":" ? (Gr.shift(), tn[rn] = tt(Gr)) : rn === "threshold" && Gr[0] === ":" ? (Gr.shift(), tn[rn] = y(Gr, x)) : fe(ze, "htmx:syntax:error", { token: Gr.shift() });
              }
              Zr.push(tn);
            }
        ...
        if (Q.config.scrollIntoViewOnBoost && ae(ze).boosted && !br(ze) && (Zr.show = "top"), Jr) {
          ...
          if (Gr.length > 0)
            for (var Yr = 0; Yr < Gr.length; Yr++) {
              ...
              } else Yr == 0 ? Zr.swapStyle = Wr : b("Unknown modifier in hx-swap: " + Wr);
            }
        }
        return Zr;
      }
